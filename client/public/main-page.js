...
// Function to fetch devices, with optional retry on failure
async function fetchDevices(retry = true) {
  try {
    ...
    // Retry once after a short delay if session may not have loaded yet
    if (retry) {
...
// Call fetchDevices after DOM is fully loaded
window.addEventListener("DOMContentLoaded", () => {
  const token = localStorage.getItem("authToken");
  if (!token) {
    window.location.href = "/login.html"; // Redirect if not logged in
    ...
    document.getElementById("content").style.display = "block";
    fetchDevices(); // Attempt to fetch devices on initial load
  }
});
